                        ...
                        <div
                            ...
                            <div class="d-flex align-items-center mb-3 mb-md-0 w-100">
                                ...
                                <div class="flex-shrink-0 me-3">
                                    <a
                                        href="{{ route('detail', ['id' => $history->room->boardingHouse->id ?? 0, 'room_id' => $history->room->id ?? 0]) }}">
                                        @if (isset($history->room) && $history->room->main_image)
                                            <img src="{{ asset($history->room->main_image) }}"
                                                ...
                                                style="width: 100px; height: 100px;" alt="Kamar Kost">
                                        @else
                                            <div class="rounded-3 d-flex align-items-center justify-content-center bg-light text-muted border shadow-sm"
                                ...
                                <div class="w-100">
                                    <div class="d-flex justify-content-between align-items-start mb-1">
                                        <a href="{{ route('detail', ['id' => $history->room->boardingHouse->id ?? 0, 'room_id' => $history->room->id ?? 0]) }}"
                                            ...
                                            <h5 class="fw-bold mb-0 text-dark hover-primary"
                                                style="font-size: 1.1rem; transition: color 0.2s;">
                                                {{ $history->room->boardingHouse->boarding_house_name ?? 'Nama Kost Tidak Diketahui' }}
                                            </h5>
                                        </a>
                                    </div>

                                    <p class="text-muted mb-2 small">{{ $history->room->room_type ?? 'Tipe Kamar' }} -
                                        {{ $history->room->room_name ?? 'No.' }}</p>

                                    @php
                                        $boardingHouseId = $history->room->boardingHouse->id ?? null;
                                        $hasReviewed =
                                            $boardingHouseId && isset($reviews) && $reviews->has($boardingHouseId);
                                    @endphp
                                    ...
                                    <div class="mt-2 dropdown">
                                        @if ($hasReviewed)
                                            ...
                                            @endphp
                                            <div class="d-flex align-items-center gap-1 text-warning mt-2"
                            ...
                            <div class="text-md-end text-start d-flex flex-column justify-content-between align-items-md-end h-100 flex-shrink-0"
                                ...
                                <div class="d-none d-md-block mb-3 text-end">
                                    @if (in_array(strtolower($history->status), ['active', 'sedang disewa']))
                                        <span class="badge bg-primary text-white px-3 py-1 rounded-pill"
                                            style="font-size: 0.7rem; font-weight: 600;">SEDANG DISEWA</span>
                                    @elseif(in_array(strtolower($history->status), ['completed', 'selesai']))
                                        <span class="badge bg-success text-white px-3 py-1 rounded-pill"
                                            style="font-size: 0.7rem; font-weight: 600;">SELESAI</span>
                                    @else
                                        <span class="badge bg-secondary text-white px-3 py-1 rounded-pill"
                                            style="font-size: 0.7rem; font-weight: 600;">{{ strtoupper($history->status) }}</span>
                                    @endif
                                </div>

                                <div
                                    class="mt-auto d-flex flex-md-column justify-content-between align-items-center align-items-md-end w-100">
                                    <h5 class="text-primary fw-bold mb-md-2 mb-0 fs-5">Rp
                                        {{ number_format($history->payments->first()->amount ?? 0, 0, ',', '.') }}</h5>
            ...
            starContainers.forEach(container => {
                ...
                stars.forEach(star => {
                    ...
                    star.addEventListener('mouseout', () => {
                        stars.forEach(s => {
                            s.style.color = parseInt(s.getAttribute(
                                    'data-value')) <= selectedRating ? '#FBBF24' :
                                '#e5e7eb';
                        });
                    });
