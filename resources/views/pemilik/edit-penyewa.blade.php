...
@section('content')
<style>
    .container-fluid-custom { width: 100%; max-width: 780px; margin: 0 auto; padding: 0 16px; }
    .form-label-custom { font-size: 0.75rem; font-weight: 700; color: #495057; margin-bottom: 8px; text-transform: uppercase; letter-spacing: 0.5px; display: block; }
    .form-control-custom { background-color: #f1f3f5; border: 1px solid #f1f3f5; border-radius: 8px; padding: 12px 16px; font-size: 0.9rem; color: #495057; transition: all 0.2s; width: 100%; }
    ...
    .form-section-card { background: #fff; border-radius: 16px; padding: 28px; margin-bottom: 20px; box-shadow: 0 4px 10px rgba(0,0,0,0.03); border: 1px solid #f1f3f5; }
    .section-title-wrap { display: flex; align-items: center; gap: 12px; margin-bottom: 20px; }
        ...
        <div class="form-section-card">
            ...
            <div class="row g-3">
                <div class="col-12 col-md-6">
                    ...
                    <input type="text" class="form-control-custom" value="{{ $tenant->user->name ?? '-' }}" readonly style="background-color: #f8f9fa; color: #6c757d;">
                </div>
                <div class="col-12 col-md-6">
                    <label class="form-label-custom">Email</label>
                    <input type="text" class="form-control-custom" value="{{ $tenant->user->email ?? '-' }}" readonly style="background-color: #f8f9fa; color: #6c757d;">
                </div>
            </div>
        </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-12 col-md-6">
                    ...
                    <select name="rental_type" class="form-control-custom" required>
                        <option value="daily"   {{ $tenant->rental_type === 'daily'   ? 'selected' : '' }}>Harian</option>
                        <option value="weekly"  {{ $tenant->rental_type === 'weekly'  ? 'selected' : '' }}>Mingguan</option>
                        <option value="monthly" {{ $tenant->rental_type === 'monthly' ? 'selected' : '' }}>Bulanan</option>
                    </select>
                </div>
                <div class="col-12 col-md-6">
                    <label class="form-label-custom">Tanggal Masuk</label>
                    <input type="date" name="start_date" class="form-control-custom" value="{{ $tenant->start_date?->format('Y-m-d') }}" required>
                </div>
                <div class="col-12 col-md-6">
                    <label class="form-label-custom">Tanggal Keluar <span class="text-muted fw-normal" style="text-transform:none;">(opsional)</span></label>
                    <input type="date" name="end_date" class="form-control-custom" value="{{ $tenant->end_date?->format('Y-m-d') }}">
                </div>
                <div class="col-12 col-md-6">
                    <label class="form-label-custom">Total Bayar (Rp)</label>
                    <input type="number" name="total_price" class="form-control-custom" value="{{ $tenant->total_price }}">
                </div>
                <div class="col-12 col-md-6">
                    ...
                    <select name="status" class="form-control-custom" required>
                        ...
                        <option value="complete"  {{ $tenant->status === 'complete'  ? 'selected' : '' }}>Selesai</option>
                        <option value="cancelled" {{ $tenant->status === 'cancelled' ? 'selected' : '' }}>Terminated</option>
                    </select>
                </div>
            </div>
        </div>
